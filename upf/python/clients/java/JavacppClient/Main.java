...
class Main {
    ...
    public static void main(String[] args) {
        UPF.init();
        int size = 14;
        UPF.Problem p = generateProblem(size);
        UPF.StringSet goal = new UPF.StringSet();
        for (int i = 0; i < size; i++) {
            String name = "v"+i;
            goal.insert(new BytePointer(name));
        ...
        GoalCounter h = new GoalCounter(goal);

        List<String> planners = List.of("cppplanner_upf", "pyplanner_upf", "jplanner_upf", "jplanner_upf_grpc");
