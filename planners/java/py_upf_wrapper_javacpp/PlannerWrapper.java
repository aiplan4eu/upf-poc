...
@Platform(includepath={},
          linkpath = {},
          link = {},
          include={"<optional>", "<set>", "<string>", "<vector>", "heuristic.hxx"},
          compiler={"cpp17"})
public class PlannerWrapper {
    static { Loader.load(); }

    @NoOffset public static class Action extends Pointer {
        static { Loader.load(); }
    ...
    @Name("std::set<std::string>") public static class StringSet extends Pointer {
        ...
        public Set<String> toSet() {
            ...
            for (Iterator i=begin(); i!=end(); i.increment()) {
                res.add(i.get().toString());
            }
            return res;
        }
    }

    @Name("std::vector<Action>") public static class ActionVector extends Pointer {
        static { Loader.load(); }
        /** Pointer cast constructor. Invokes {@link Pointer#Pointer(Pointer)}. */
    ...
    @NoOffset public static class Problem extends Pointer {
        static { Loader.load(); }
        /** Pointer cast constructor. Invokes {@link Pointer#Pointer(Pointer)}. */
